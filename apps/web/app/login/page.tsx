...
function LoginForm() {
  ...
  // Check if user already has tokens (in cookies)
  ...
  useEffect(() => {
    ...
    if (accessToken || refreshToken || user) {
      // Tokens are already in cookies (tokenStorage uses cookies)
      // If we have tokens, redirect to the intended destination
    ...
    <Card className="w-full max-w-md bg-slate-900 border-slate-800 text-white shadow-2xl">
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Button
            type="submit"
            className="w-full bg-emerald-600 hover:bg-emerald-700 font-bold mt-4 h-12 text-lg"
            disabled={loading}
          >
