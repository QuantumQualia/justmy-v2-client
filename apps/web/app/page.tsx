...
export default function LandingPage() {
  ...
  const STRIPE_PRICES = {
    ...
    FOUNDER: process.env.NEXT_PUBLIC_STRIPE_PRICE_FOUNDER || ""
  };
  ...
  const handleCheckout = async (priceId: string) => {
    ...
    try {
      ...
      else alert("System connecting... (Verify API Keys)");
    } catch (error) {
      console.error(error);
      ...
      <nav className="border-b border-white/10 bg-black/50 backdrop-blur-md sticky top-0 z-50">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex gap-4">
            <Button variant="ghost" className="text-slate-400 hover:text-white" onClick={() => window.open("https://old.justmy.com/login", "_blank")}>
              Legacy Login
            </Button>
            <Button 
      ...
      <section className="relative pt-20 pb-32 overflow-hidden">
        ...
        <div className="container px-4 mx-auto text-center relative z-10">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            <Button size="lg" className="h-14 px-8 text-lg bg-white text-black hover:bg-slate-200 rounded-full font-bold transition-transform hover:scale-105" onClick={() => document.getElementById('pricing')?.scrollIntoView({behavior: 'smooth'})}>
        ...
        <div className="container px-4 mx-auto max-w-7xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <PricingCard 
              ...
              btnAction={() => handleCheckout(STRIPE_PRICES.GROWTH)}
            ...
            <div className="relative transform md:-translate-y-4">
              ...
              <Card className="bg-slate-900 border-2 border-emerald-500 shadow-2xl shadow-emerald-900/20 h-full">
                <CardContent className="p-6 flex flex-col h-full">
                  ...
                  <Button 
                    className="w-full bg-emerald-500 hover:bg-emerald-600 text-black font-bold mb-6"
                    onClick={() => handleCheckout(STRIPE_PRICES.FOUNDER)}
                    disabled={loading}
                  >
    ...
    <Card className={`bg-slate-900/50 border-slate-800 text-slate-400 h-full ${isPopular ? 'border-blue-500/50' : ''}`}>
      <CardContent className="p-6 flex flex-col h-full">
        ...
        <Button 
          ...
          className={`w-full mb-6 ${isPopular ? 'bg-blue-600 hover:bg-blue-700 text-white font-bold' : 'border-slate-700 hover:bg-slate-800 hover:text-white'}`}
          onClick={btnAction}
          disabled={loading}
        >
