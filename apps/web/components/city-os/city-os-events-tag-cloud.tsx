...
import type { CityOsEventDto } from "@/lib/api/cityos-events";

import "./cityos-events-cloud.css";
...
export interface CityOsEventsTagCloudProps {
  ...
  /** `light` = white card (embed). `dark` = navy card (CMS / in-app). */
  variant?: "light" | "dark";
  className?: string;
}

/** Extra fields preserved on words through layout. */
...
/**
 * CityOS events — **`WordCloud`** from `@isoterik/react-word-cloud`. Section title is **`words[0]`**
 * (**THIS WEEK IN [marketCity]** when the API sends `marketCity`). Font stack uses **Fonarto** (see `cityos-events-cloud.css`).
 */
